  ...
  return (
    <div className="flex items-center justify-between p-4 border rounded-2xl shadow hover:bg-gray-50 transition mb-4">
        ...
        <div className="flex w-full gap-x-2">
          ...
          <div>
            {event.isRecentlyAdded && (
              <Badge variant="secondary" className="bg-red-600">Recently added</Badge>
            )}
          </div>
        </div>
      </div>
      <Badge
        className={clsx(
          "p-2 px-3",
          event.status === "Not Started" && "border-gray-400 text-gray-700 bg-gray-100",
          event.status === "Ongoing" && "border-red-500 text-red-500 bg-red-50",
          event.status === "Finished" && "border-green-500 text-green-500 bg-green-50"
        )}
      >
        {event.status}
      </Badge>
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => navigate("/create-event")} className="bg-[#4285F4] hover:bg-[#2469DB]">+ Add New Event</Button>
      </div>

      <div className="flex flex-col md:flex-row gap-4 mb-10">
        <div className="flex-1">
          <label className="block mb-1 text-sm font-medium">Search by Years</label>
      ...
      {events.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center py-16">
          ...
          <Button onClick={() => navigate("/create-event")} className="mt-4 bg-[#4285F4] hover:bg-[#2469DB] rounded-full">Create Event</Button>
        </div>
      ) : (
        <div>
